...
For Claude Code users who don't want to run the service. Only needs `yt-dlp` installed (`brew install yt-dlp`). Extracts YouTube subtitles directly — no Whisper, no library, but auto-upgrades to the full service when it detects it running.
